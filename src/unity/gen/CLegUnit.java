...
public class CLegUnit extends Unit implements Statusc, Entityc, Drawc, Hitboxc, Physicsc, Itemsc, Flyingc, Shieldc, Posc, Commanderc, Syncc, CLegc, Weaponsc, Teamc, Unitc, Healthc, Rotc, Boundedc, Builderc, Minerc, Velc {
    ...
    public transient CLegGroup[] legGroups;
    ...
    public void add() {
        if (!this.added) {
            ...
            this.legGroups = new CLegGroup[uType.legGroup.size];

            for(int i = 0; i < this.legGroups.length; ++i) {
                this.legGroups[i] = ((CLegType.ClegGroupType)uType.legGroup.get(i)).create();
                this.legGroups[i].reset(this);
            }
    ...
    public void update() {
        ...
        this.floor = Vars.world.floorWorld(this.x(), this.y());
        boolean allNull = this.floor != null && this.floor != Blocks.air;

        for(CLegGroup group : this.legGroups) {
            group.update(this);
            this.floor = group.lastFloor;
            if (!allNull) {
                allNull = group.lastFloor == null;
            }
        }
    ...
    public Floor drownFloor() {
        return this.canDrown() ? this.floor : null;
    }
    ...
    public static CLegUnit create() {
        return new CLegUnit();
    }

    public int classId() {
        return UnityEntityMapping.classId(CLegUnit.class);
    }

    public CLegGroup[] legGroups() {
        return this.legGroups;
    }

    public void legGroups(CLegGroup[] legGroups) {
