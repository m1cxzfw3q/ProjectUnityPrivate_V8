...
public class CopterUnit extends Unit implements Statusc, Entityc, Drawc, Hitboxc, Physicsc, Itemsc, Flyingc, Shieldc, Posc, Commanderc, Syncc, Weaponsc, Teamc, Unitc, Healthc, Rotc, Boundedc, Copterc, Builderc, Minerc, Velc {
    ...
    public transient RotorMount[] rotors;
    public transient float rotorSpeedScl;
    ...
    protected CopterUnit() {
        ...
        this.rotorSpeedScl = 1.0F;
        this.buildAlpha = 0.0F;
    }

    public String toString() {
        return "CopterUnit#" + this.id;
    }
    ...
    public void add() {
        if (!this.added) {
            ...
            this.rotors = new RotorMount[type.rotors.size];

            for(int i = 0; i < this.rotors.length; ++i) {
                Rotor rotor = (Rotor)type.rotors.get(i);
                this.rotors[i] = new RotorMount(rotor);
                this.rotors[i].rotorRot = rotor.rotOffset;
                this.rotors[i].rotorShadeRot = rotor.rotOffset;
            }
    ...
    public void update() {
        ...
        UnityUnitType type = (UnityUnitType)this.type;
        if (!this.dead && !(this.health < 0.0F)) {
            this.rotorSpeedScl = Mathf.lerpDelta(this.rotorSpeedScl, 1.0F, type.rotorDeathSlowdown);
        } else {
            if (!Vars.net.client() || this.isLocal()) {
                this.rotation += type.fallRotateSpeed * (float)Mathf.signs[this.id % 2] * Time.delta;
            }

            this.rotorSpeedScl = Mathf.lerpDelta(this.rotorSpeedScl, 0.0F, type.rotorDeathSlowdown);
        }

        for(RotorMount rotor : this.rotors) {
            rotor.rotorRot += rotor.rotor.speed * this.rotorSpeedScl * Time.delta;
            rotor.rotorRot %= 360.0F;
            rotor.rotorShadeRot += rotor.rotor.shadeSpeed * Time.delta;
            rotor.rotorShadeRot %= 360.0F;
        }
    ...
    public int classId() {
        return UnityEntityMapping.classId(CopterUnit.class);
    }
    ...
    public void rotors(RotorMount[] rotors) {
        this.rotors = rotors;
    }

    public float rotorSpeedScl() {
        return this.rotorSpeedScl;
    }

    public void rotorSpeedScl(float rotorSpeedScl) {
        this.rotorSpeedScl = rotorSpeedScl;
    }
}
