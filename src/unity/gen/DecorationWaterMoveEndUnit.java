...
public class DecorationWaterMoveEndUnit extends Unit implements Drawc, Hitboxc, Itemsc, Commanderc, Endc, Weaponsc, Healthc, Builderc, Velc, Statusc, Entityc, Physicsc, Flyingc, Posc, Rotc, Shieldc, Decorationc, Teamc, Unitc, Factionc, WaterMovec, Boundedc, Minerc, Syncc {
    ...
    public transient UnitDecorationType.UnitDecoration[] decors;
    private UnitController controller;
    private transient float resupplyTime;
    private transient boolean wasPlayer;
    private transient boolean wasHealed;
    private transient Trail tleft;
    private transient Trail tright;
    private transient Color trailColor;

    protected DecorationWaterMoveEndUnit() {
        this.applied = new Bits(Vars.content.getBy(ContentType.status).size);
        this.decors = new UnitDecorationType.UnitDecoration[0];
    ...
    public String toString() {
        return "DecorationWaterMoveEndUnit#" + this.id;
    }
    ...
    public void draw() {
        ...
        Draw.z(z);
        if (this.mining()) {
            z = this.hitSize / 2.0F + Mathf.absin(Time.time, 1.1F, 0.5F);
            float swingScl = 12.0F;
            float swingMag = 1.0F;
            float flashScl = 0.3F;
            float px = this.x + Angles.trnsx(this.rotation, z);
            float py = this.y + Angles.trnsy(this.rotation, z);
    ...
    public void add() {
        if (!this.added) {
            ...
            for(UnitDecorationType.UnitDecoration decor : this.decors) {
                decor.added(this);
            }
    ...
    public void setType(UnitType type) {
        ...
        this.decors = new UnitDecorationType.UnitDecoration[uType.decorations.size];

        for(int i = 0; i < uType.decorations.size; ++i) {
            this.decors[i] = (UnitDecorationType.UnitDecoration)((UnitDecorationType)uType.decorations.get(i)).decalType.get((UnitDecorationType)uType.decorations.get(i));
        }
    ...
    public static DecorationWaterMoveEndUnit create() {
        return new DecorationWaterMoveEndUnit();
    }

    public int classId() {
        return UnityEntityMapping.classId(DecorationWaterMoveEndUnit.class);
    }
    ...
    public UnitDecorationType.UnitDecoration[] decors() {
        return this.decors;
    }

    public void decors(UnitDecorationType.UnitDecoration[] decors) {
        this.decors = decors;
    }
}
