...
public class ExpForceProjector extends ForceProjector {
    ...
    @Nullable
    public ExpForceProjector pregrade = null;
    ...
    public ExpForceProjector(String name) {
        super(name);
            ...
            if (core != null && (Vars.state.rules.infiniteResources || core.items.has(this.requirements, Vars.state.rules.buildCostMultiplier))) {
                ...
                tile.getLinkedTilesAs(this, (inside) -> {
                    if (inside.build != null && !this.seqs.contains(inside.build) && this.seqs.size <= 1) {
                        if (inside.block() == this.pregrade && ((ExpForceProjectorBuild)inside.build).level() >= this.pregradeLevel) {
    ...
    public class ExpForceProjectorBuild extends ForceProjector.ForceBuild implements ExpHolder, LevelHolder {
        public int exp;
        @Nullable
        public ExpHub.ExpHubBuild hub = null;

        public ExpForceProjectorBuild() {
            super(ExpForceProjector.this);
        }

        public String toString() {
            return "ExpForceProjectorBuild#" + this.id;
        }

        public int maxLevel() {
            return ExpForceProjector.this.maxLevel;
        }

        public void read(Reads read, byte revision) {
            super.read(read, revision);
            this.exp = read.i();
            if (this.exp > ExpForceProjector.this.maxExp) {
                this.exp = ExpForceProjector.this.maxExp;
            }

        }

        public int level() {
            return ExpForceProjector.this.expLevel(this.exp);
        }

        public int incExp(int amount, boolean hub) {
            int ehub = hub && this.hubValid() ? this.hub.takeAmount(amount, this) : 0;
            int e = Math.min(amount - ehub, ExpForceProjector.this.maxExp - this.exp);
            if (e == 0) {
                return 0;
            } else {
        ...
        public Color shootColor(Color tmp) {
            return tmp.set(ExpForceProjector.this.fromColor).lerp(ExpForceProjector.this.toColor, (float)this.exp / (float)ExpForceProjector.this.maxExp);
        }

        public void update() {
            if (ExpForceProjector.this.updateExpFields) {
                ExpForceProjector.this.setEFields(this.level());
            }
        ...
        public boolean handleOrb(int orbExp) {
            int a = (int)(ExpForceProjector.this.orbScale * (float)orbExp);
            if (a < 1) {
                return false;
            } else {
                this.incExp(a, false);
        ...
        public void buildHBar(Table t) {
            ...
            int l = this.level();
            if ((Float)ExpForceProjector.this.damageReduction.fromLevel(this.level()) >= 0.01F) {
                Image ii = new Image(Icon.defense, Pal.health);
                ii.setSize(14.0F);
                Label ll = new Label(() -> Mathf.roundPositive((Float)ExpForceProjector.this.damageReduction.fromLevel(this.level()) * 100.0F) + "");
        ...
        public Color effectColor() {
            return ExpForceProjector.this.effectColors == null ? Color.white : ExpForceProjector.this.effectColors[Math.min((int)(this.levelf() * (float)ExpForceProjector.this.effectColors.length), ExpForceProjector.this.effectColors.length - 1)];
        }
        ...
        public float handleDamage(float amount) {
            return super.handleDamage(amount) * Mathf.clamp(1.0F - (Float)ExpForceProjector.this.damageReduction.fromLevel(this.level()));
        }

        public void levelup() {
            ExpForceProjector.this.upgradeSound.at(this);
            ExpForceProjector.this.upgradeEffect.at(this);
            if (ExpForceProjector.this.upgradeBlockEffect != Fx.none) {
                ExpForceProjector.this.upgradeBlockEffect.at(this.x, this.y, (float)(this.rotation - 90), Color.white, ExpForceProjector.this.region);
            }

        }

        public float levelf() {
            return (float)this.level() / (float)ExpForceProjector.this.maxLevel;
        }

        public int unloadExp(int amount) {
            if (ExpForceProjector.this.passive) {
                return 0;
            } else {
        ...
        public void displayBars(Table table) {
            ...
            table.table((t) -> {
                t.defaults().height(18.0F).pad(4.0F);
                t.label(() -> "Lv " + this.level()).color(ExpForceProjector.this.passive ? UnityPal.passive : Pal.accent).width(65.0F);
                t.add(new Bar(() -> this.level() >= ExpForceProjector.this.maxLevel ? "MAX" : Core.bundle.format("bar.expp", new Object[]{(int)(this.expf() * 100.0F)}), () -> UnityPal.exp, this::expf)).growX();
        ...
        public float expf() {
            int lv = this.level();
            if (lv >= ExpForceProjector.this.maxLevel) {
                return 1.0F;
            } else {
                float lb = ExpForceProjector.this.expCap(lv - 1);
                float lc = ExpForceProjector.this.expCap(lv);
                return ((float)this.exp - lb) / (lc - lb);
            }
        }

        public int handleTower(int amount, float angle) {
            return ExpForceProjector.this.passive ? 0 : this.incExp(amount, false);
        }

        public void drawLight() {
            if (ExpForceProjector.this.draw != null) {
                ExpForceProjector.this.draw.drawLight(this);
            }
        ...
        public void draw() {
            if (ExpForceProjector.this.draw != null) {
                ExpForceProjector.this.draw.draw(this);
            }
        ...
        public boolean acceptOrb() {
            return !ExpForceProjector.this.passive && this.exp < ExpForceProjector.this.maxExp;
        }

        public boolean hubbable() {
            return !ExpForceProjector.this.passive;
        }

        public void killed() {
            ExpOrbs.spreadExp(this.x, this.y, (float)this.exp * 0.3F, 3.0F * (float)ExpForceProjector.this.size);
        }
    }
}
