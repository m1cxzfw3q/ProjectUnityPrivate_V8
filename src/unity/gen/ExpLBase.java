...
public class ExpLBase extends Block {
            ...
            if (core != null && (Vars.state.rules.infiniteResources || core.items.has(this.requirements, Vars.state.rules.buildCostMultiplier))) {
                ...
                tile.getLinkedTilesAs(this, (inside) -> {
                    if (inside.build != null && !this.seqs.contains(inside.build) && this.seqs.size <= 1) {
                        if (inside.block() == this.pregrade && ((ExpLBaseBuild)inside.build).level() >= this.pregradeLevel) {
    ...
    public class ExpLBaseBuild extends Building implements ExpHolder, LevelHolder {
        public int exp;
        @Nullable
        public ExpHub.ExpHubBuild hub = null;

        public String toString() {
            return "ExpLBaseBuild#" + this.id;
        }

        public int maxLevel() {
            return ExpLBase.this.maxLevel;
        }

        public void read(Reads read, byte revision) {
            super.read(read, revision);
            this.exp = read.i();
            if (this.exp > ExpLBase.this.maxExp) {
                this.exp = ExpLBase.this.maxExp;
            }

        }

        public int level() {
            return ExpLBase.this.expLevel(this.exp);
        }

        public int incExp(int amount, boolean hub) {
            int ehub = hub && this.hubValid() ? this.hub.takeAmount(amount, this) : 0;
            int e = Math.min(amount - ehub, ExpLBase.this.maxExp - this.exp);
            if (e == 0) {
                return 0;
            } else {
        ...
        public Color shootColor(Color tmp) {
            return tmp.set(ExpLBase.this.fromColor).lerp(ExpLBase.this.toColor, (float)this.exp / (float)ExpLBase.this.maxExp);
        }

        public void update() {
            if (ExpLBase.this.updateExpFields) {
                ExpLBase.this.setEFields(this.level());
            }
        ...
        public boolean handleOrb(int orbExp) {
            int a = (int)(ExpLBase.this.orbScale * (float)orbExp);
            if (a < 1) {
                return false;
            } else {
                this.incExp(a, false);
        ...
        public void buildHBar(Table t) {
            ...
            int l = this.level();
            if ((Float)ExpLBase.this.damageReduction.fromLevel(this.level()) >= 0.01F) {
                Image ii = new Image(Icon.defense, Pal.health);
                ii.setSize(14.0F);
                Label ll = new Label(() -> Mathf.roundPositive((Float)ExpLBase.this.damageReduction.fromLevel(this.level()) * 100.0F) + "");
        ...
        public Color effectColor() {
            return ExpLBase.this.effectColors == null ? Color.white : ExpLBase.this.effectColors[Math.min((int)(this.levelf() * (float)ExpLBase.this.effectColors.length), ExpLBase.this.effectColors.length - 1)];
        }
        ...
        public float handleDamage(float amount) {
            return super.handleDamage(amount) * Mathf.clamp(1.0F - (Float)ExpLBase.this.damageReduction.fromLevel(this.level()));
        }

        public void levelup() {
            ExpLBase.this.upgradeSound.at(this);
            ExpLBase.this.upgradeEffect.at(this);
            if (ExpLBase.this.upgradeBlockEffect != Fx.none) {
                ExpLBase.this.upgradeBlockEffect.at(this.x, this.y, (float)(this.rotation - 90), Color.white, ExpLBase.this.region);
            }

        }

        public float levelf() {
            return (float)this.level() / (float)ExpLBase.this.maxLevel;
        }

        public int unloadExp(int amount) {
            if (ExpLBase.this.passive) {
                return 0;
            } else {
        ...
        public void displayBars(Table table) {
            ...
            table.table((t) -> {
                t.defaults().height(18.0F).pad(4.0F);
                t.label(() -> "Lv " + this.level()).color(ExpLBase.this.passive ? UnityPal.passive : Pal.accent).width(65.0F);
                t.add(new Bar(() -> this.level() >= ExpLBase.this.maxLevel ? "MAX" : Core.bundle.format("bar.expp", new Object[]{(int)(this.expf() * 100.0F)}), () -> UnityPal.exp, this::expf)).growX();
        ...
        public float expf() {
            int lv = this.level();
            if (lv >= ExpLBase.this.maxLevel) {
                return 1.0F;
            } else {
                float lb = ExpLBase.this.expCap(lv - 1);
                float lc = ExpLBase.this.expCap(lv);
                return ((float)this.exp - lb) / (lc - lb);
            }
        }

        public int handleTower(int amount, float angle) {
            return ExpLBase.this.passive ? 0 : this.incExp(amount, false);
        }

        public void drawLight() {
            if (ExpLBase.this.draw != null) {
                ExpLBase.this.draw.drawLight(this);
            }
        ...
        public boolean acceptOrb() {
            return !ExpLBase.this.passive && this.exp < ExpLBase.this.maxExp;
        }

        public boolean hubbable() {
            return !ExpLBase.this.passive;
        }

        public void killed() {
            ExpOrbs.spreadExp(this.x, this.y, (float)this.exp * 0.3F, 3.0F * (float)ExpLBase.this.size);
        }
    }
}
