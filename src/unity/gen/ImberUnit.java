...
public class ImberUnit extends Unit implements Statusc, Entityc, Drawc, Hitboxc, Physicsc, Itemsc, Flyingc, Shieldc, Posc, Commanderc, Syncc, Weaponsc, Teamc, Unitc, Healthc, Rotc, Imberc, Boundedc, Builderc, Minerc, Velc {
    ...
    public transient Seq<Unit> closeImberUnits;
    public transient Seq<Building> closeNodes;
    public transient float laserRange;
    public transient int maxConnections;
    public transient int connections;
    ...
    protected ImberUnit() {
        ...
        this.closeImberUnits = new Seq();
        this.closeNodes = new Seq();
        this.buildAlpha = 0.0F;
    }

    public String toString() {
        return "ImberUnit#" + this.id;
    }
    ...
    public void draw() {
        ...
        for(Unit unit : this.closeImberUnits) {
            Lines.line(this.x, this.y, unit.x, unit.y);
        }

        for(Building building : this.closeNodes) {
            Lines.line(this.x, this.y, building.x, building.y);
        }
    ...
    public void update() {
        ...
        this.closeImberUnits.clear();
        this.closeNodes.clear();
        this.connections = 0;
        Units.nearby(this.team, this.x, this.y, this.laserRange, this.laserRange, (e) -> {
            if (this.connections < this.maxConnections && !e.dead && e.team == this.team) {
                UnitType unitType$temp = e.type;
                if (unitType$temp instanceof UnityUnitType) {
                    UnityUnitType unitType = (UnityUnitType)unitType$temp;
                    if (unitType.maxConnections > 0 && !((Imberc)e).closeImberUnits().contains(this.controller().unit())) {
                        this.closeImberUnits.add(e);
                        ++this.connections;
                    }
                }
            }
        ...
        Vars.indexer.eachBlock(this.team, this.x, this.y, this.laserRange, (e) -> this.connections < this.maxConnections && e.team == this.team && !e.dead && e instanceof PowerNode.PowerNodeBuild, (e) -> {
            this.closeNodes.add(e);
            ++this.connections;
        });
        if (!Vars.net.client() || this.isLocal()) {
            float dx = 0.0F;
    ...
    public static ImberUnit create() {
        return new ImberUnit();
    }

    public int classId() {
        return UnityEntityMapping.classId(ImberUnit.class);
    }
    ...
    public Seq<Unit> closeImberUnits() {
        return this.closeImberUnits;
    }

    public void closeImberUnits(Seq<Unit> closeImberUnits) {
        this.closeImberUnits = closeImberUnits;
    }

    public Seq<Building> closeNodes() {
        return this.closeNodes;
    }

    public void closeNodes(Seq<Building> closeNodes) {
    ...
    public void laserRange(float laserRange) {
