...
public class KamiLaser extends Bullet implements Pool.Poolable, Entityc, Drawc, Timedc, Ownerc, Posc, Hitboxc, Damagec, Bulletc, Teamc, Timerc, Shielderc, Velc, KamiLaserc {
    ...
    public KamiBulletDatas.KamiLaserData bdata;

    protected KamiLaser() {
    }

    public String toString() {
        return "KamiLaser#" + this.id;
    }
    ...
    public void update() {
        ...
        if (!this.intervalCollision || this.timer(1, 5.0F)) {
            this.updateCollision();
        }

        if ((this.collidedTime += Time.delta) >= 60.0F) {
            this.collided.clear();
    ...
    public void hitbox(Rect rect) {
        ...
        rect.setCentered(this.x, this.y, 0.0F);
    }
    ...
    public void updateCollision() {
        float mx = (this.x + this.x2) / 2.0F;
        float my = (this.y + this.y2) / 2.0F;
        float ang = Angles.angle(this.x, this.y, this.x2, this.y2);
        Tmp.r1.setCentered(this.x, this.y, this.width * 2.0F);
        Tmp.r2.setCentered(this.x2, this.y2, this.width * 2.0F);
        Tmp.r1.merge(Tmp.r2);

        for(Teams.TeamData data : Vars.state.teams.present) {
            if (data.team != this.team && data.tree != null) {
                data.tree.intersect(Tmp.r1, (e) -> {
                    if (!this.collided.contains(e.id)) {
                        float size = e.hitSize / 2.0F;
                        if (this.ellipseCollision) {
                            v.set(e).sub(mx, my).rotate(-ang);
                            ep.set(0.0F, 0.0F, Mathf.dst(this.x, this.y, this.x2, this.y2) * 2.0F + size, this.width * 2.0F + size);
                            if (ep.contains(v)) {
                                this.type.hitEntity(this, e, e.health);
                                this.type.hit(this, e.x, e.y);
                                this.collided.add(e.id);
                            }
                        } else {
                            float dst = Intersector.distanceSegmentPoint(this.x, this.y, this.x2, this.y2, e.x, e.y);
                            if (dst < this.width + size) {
                                this.type.hitEntity(this, e, e.health);
                                this.type.hit(this, e.x, e.y);
                                this.collided.add(e.id);
                            }
    ...
    public void reset() {
        ...
        this.lastTime = 0.0F;
        this.collidedTime = 0.0F;
        this.intervalCollision = true;
        this.ellipseCollision = true;
        this.bdata = null;
    }

    public static KamiLaser create() {
        return (KamiLaser)Pools.obtain(KamiLaser.class, KamiLaser::new);
    }

    public int classId() {
        return UnityEntityMapping.classId(KamiLaser.class);
    }
    ...
    public void collidedTime(float collidedTime) {
        this.collidedTime = collidedTime;
    }

    public boolean intervalCollision() {
        return this.intervalCollision;
    }

    public void intervalCollision(boolean intervalCollision) {
        this.intervalCollision = intervalCollision;
    }

    public boolean ellipseCollision() {
        return this.ellipseCollision;
    }

    public void ellipseCollision(boolean ellipseCollision) {
        this.ellipseCollision = ellipseCollision;
    }
    ...
    public void bdata(KamiBulletDatas.KamiLaserData bdata) {
        this.bdata = bdata;
    }
}
