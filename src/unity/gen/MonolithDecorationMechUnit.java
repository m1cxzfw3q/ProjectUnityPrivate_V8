...
public class MonolithDecorationMechUnit extends Unit implements ElevationMovec, Drawc, Hitboxc, Itemsc, Rotc, Commanderc, Weaponsc, Mechc, Healthc, Builderc, Velc, Statusc, Entityc, Physicsc, Flyingc, Posc, Monolithc, Shieldc, Decorationc, Teamc, Unitc, Factionc, Boundedc, Minerc, Syncc {
    ...
    protected MonolithDecorationMechUnit() {
    ...
    public String toString() {
        return "MonolithDecorationMechUnit#" + this.id;
    }
    ...
    public void update() {
        ...
        if (this.updateBuilding && this.canBuild()) {
            extend = Vars.state.rules.infiniteResources ? Float.MAX_VALUE : 220.0F;
            ...
            if (this.buildPlan() != null) {
                ...
                if (this.plans.size > 1) {
                    for(int total = 0; (!this.within((req = this.buildPlan()).tile(), extend) || this.shouldSkip(req, core)) && total < this.plans.size; ++total) {
        ...
        if (this.dead || this.health <= 0.0F) {
            ...
            if (Mathf.chanceDelta(0.2)) {
                ...
                base = Mathf.range(this.type.engineSize);
                this.type.fallThrusterEffect.at(this.x + Angles.trnsx(this.rotation + 180.0F, offset) + Mathf.range(base), this.y + Angles.trnsy(this.rotation + 180.0F, offset) + Mathf.range(base), Mathf.random());
            }
    ...
    public static MonolithDecorationMechUnit create() {
        return new MonolithDecorationMechUnit();
    }

    public int classId() {
        return UnityEntityMapping.classId(MonolithDecorationMechUnit.class);
    }
    ...
    public void walkTime(float arg0) {
        this.walkTime = this.walkTime;
    }
    ...
    public void walkExtension(float arg0) {
        this.walkExtension = this.walkExtension;
    }
