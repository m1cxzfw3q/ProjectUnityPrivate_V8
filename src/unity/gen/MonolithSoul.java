...
public class MonolithSoul extends Unit implements Drawc, Hitboxc, CTrailc, Itemsc, Commanderc, Weaponsc, MonolithSoulc, Builderc, Velc, Statusc, Entityc, Healthc, Physicsc, Flyingc, Posc, Rotc, Shieldc, Teamc, Unitc, Factionc, Boundedc, Minerc, Syncc {
    ...
    protected transient boolean corporeal;
    protected transient float joinTime;
    protected transient Teamc joinTarget;
    protected transient float ringRotation;
    protected transient Seq<Tile> forms = new Seq(5);
    protected transient float formProgress;
    ...
    public float lifeDelta() {
        return ((float)this.forms.size - 2.5F) * 0.18F;
    }

    public boolean forming() {
        return this.forms.any();
    }
    ...
    public void setType(UnitType type) {
        if (type instanceof UnityUnitType) {
            ...
            this.trail = (Trail)t.trailType.get(this);
        }
    ...
    public void destroy() {
        if (this.isAdded()) {
            TrailFx.trailFadeLow.at(this.x, this.y, (this.type.engineSize + Mathf.absin(Time.time, 2.0F, this.type.engineSize / 4.0F) * this.elevation) * this.type.trailScl, this.trail.copy());
        }
    ...
    public boolean formValid(Tile tile) {
        if (!tile.synthetic() && !(Mathf.dst(this.x, this.y, tile.getX(), tile.getY()) > this.type.range)) {
    ...
    public void add() {
        if (!this.added) {
            ...
            this.updateLastPosition();
            this.health = Math.min(this.maxHealth / 2.0F, this.health);
            this.added = true;
    ...
    public boolean joining() {
        return this.joinTarget != null && this.joinTarget.isAdded();
    }
    ...
    public static MonolithSoul create(Team team) {
    ...
    public void join(Teamc other) {
        if (this.joinValid(other)) {
            if (this.forms.any()) {
                this.forms.clear();
            }

            this.joinTarget = other;
        }
    }
    ...
    public boolean joinValid(Teamc other) {
        ...
        boolean var10000;
        if (soul != null && other.isAdded() && soul.acceptSoul(1) >= 1) {
            float var10002 = this.type.range;
            float var10003;
            if (other instanceof Unit) {
                Unit unit = (Unit)other;
                var10003 = unit.hitSize / 2.0F;
            } else if (other instanceof Building) {
                Building build = (Building)other;
                var10003 = build.hitSize() / 2.0F;
            } else {
                var10003 = 0.0F;
            }

            if (this.within(other, var10002 + var10003)) {
                var10000 = true;
                return var10000;
            }
        }
    ...
    public void update() {
        if (!this.corporeal) {
            this.health = Mathf.clamp(this.health + (this.joining() ? -0.2F : this.lifeDelta()) * Time.delta, 0.0F, this.maxHealth);
            this.joinTime = this.joinTarget != null && this.joinTarget.isAdded() ? Mathf.approachDelta(this.joinTime, 1.0F, 0.008F) : Mathf.lerpDelta(this.joinTime, 0.0F, 0.1F);
            this.formProgress = Mathf.lerpDelta(this.formProgress, this.forms.any() ? this.health / this.maxHealth : 0.0F, 0.17F);
            this.ringRotation = Mathf.slerp(this.ringRotation, this.joinTarget == null ? this.rotation : this.angleTo(this.joinTarget), 0.08F);
            if (!this.joinValid(this.joinTarget)) {
                this.joinTarget = null;
            }

            this.forms.removeAll((t) -> !this.formValid(t));
            if (this.isLocal() && !Vars.mobile) {
                float mx = Core.input.mouseWorldX();
                float my = Core.input.mouseWorldY();
                if (Core.input.keyTap(Binding.select)) {
                    Tile tile = Vars.world.tileWorld(mx, my);
                    if (tile != null) {
                        this.form(tile);
                    }
                } else if (Core.input.keyTap(Binding.break_block)) {
                    Teamc target = Units.closest(this.team, mx, my, 1.0F, this::joinValid);
                    if (target == null) {
                        target = Vars.world.buildWorld(mx, my);
                    }
        ...
        } else if (this.health <= this.maxHealth * 0.5F) {
            DeathFx.monolithSoulCrack.at(this.x, this.y, this.rotation);
        ...
        if (this.isValid()) {
            if (Mathf.equal(this.joinTime, 1.0F) && this.joinValid(this.joinTarget)) {
                this.kill();
                DeathFx.monolithSoulJoin.at(this.x, this.y, this.ringRotation, this);
                LineFx.monolithSoulTransfer.at(this.x, this.y, this.rotation, this.joinTarget);
                float var10000 = LineFx.monolithSoulTransfer.lifetime;
                Soul var10001 = Soul.toSoul(this.joinTarget);
                Objects.requireNonNull(var10001);
                Time.run(var10000, var10001::join);
            } else if (!this.corporeal && Mathf.equal(this.health, this.maxHealth)) {
                this.corporeal = true;
                this.joinTime = 0.0F;
            }
        }
        ...
        if (this.trail != null) {
            ...
            this.trail.update(this.x + Angles.trnsx(this.rotation + 180.0F, offset), this.y + Angles.trnsy(this.rotation + 180.0F, offset));
        }

    }

    public void form(Tile tile) {
        if (this.forms.contains(tile)) {
            this.forms.remove(tile);
        } else if (this.formValid(tile)) {
    ...
    public static MonolithSoul create() {
        return new MonolithSoul();
    }

    public int classId() {
        return UnityEntityMapping.classId(MonolithSoul.class);
    }
