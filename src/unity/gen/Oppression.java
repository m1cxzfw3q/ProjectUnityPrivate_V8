...
public class Oppression extends Unit implements Drawc, Hitboxc, Itemsc, Commanderc, Endc, Weaponsc, Wormc, Healthc, Builderc, Velc, Statusc, Entityc, Physicsc, Flyingc, Posc, Rotc, Shieldc, Oppressionc, Teamc, Unitc, Factionc, Boundedc, Minerc, Syncc {
    ...
    private transient float invTimer = 0.0F;
    private float resist;
    private float resistMax;
    private float resistTime;
    protected transient boolean isRotate;
    public transient Unit head;
    public transient Unit parent;
    public transient Unit child;
    public transient float layer = 0.0F;
    public transient float scanTime = 0.0F;
    public transient byte weaponIdx = 0;
    public transient boolean removing = false;
    public transient boolean saveAdd = false;
    public float splitHealthDiv = 1.0F;
    ...
    public void damage(float amount) {
        if (!this.isHead() && this.head != null && !((UnityUnitType)this.type).splittable) {
            this.head.damage(amount);
        } else {
            UnityUnitType utype = (UnityUnitType)this.type;
            AntiCheatVariables aType = utype.antiCheatType;
            if (aType != null) {
                if (!(this.invFrames[this.invIndex] <= 0.0F)) {
                    return;
                }
                ...
                if (this.invTimer <= 0.0F) {
                    ++this.invIndex;
                    this.invIndex %= this.invFrames.length;
                    this.invTimer = 3.0F;
                }
            }

            float tmpAmount = Math.max(amount - this.armor, 0.1F * amount) / this.healthMultiplier;
            if (tmpAmount > 0.0F) {
                float shieldDamage = Math.min(Math.max(this.shield, 0.0F), tmpAmount);
                tmpAmount -= shieldDamage;
                if (tmpAmount > 0.0F) {
                    this.trueHealth -= tmpAmount;
                }
            }
    ...
    private void connect(Wormc other) {
        if (this.isHead() && other.isTail()) {
            float z = other.layer() + 1.0F;
            this.distributeActionBack((u) -> {
                ((Wormc)u).layer(((Wormc)u).layer() + z);
                ((Wormc)u).head(other.head());
            });
            other.child(this);
            this.parent = (Unit)other;
            this.head = other.head();
            this.setupWeapons(this.type);
            ((UnityUnitType)this.type).chainSound.at(this);
            if (this.controller() instanceof Player) {
                UnitController con = this.controller();
    ...
    public void updateAggroSpeed() {
        this.speedMultiplier *= this.aggression + 1.0F;
    }
    ...
    public void read(Reads read) {
        this.aggression = read.f();
        this.aggressionTime = read.f();
        this.ammo = read.f();
        this.childId = read.i();
        ...
        this.headId = read.i();
        this.health = read.f();
        int invFrames_LENGTH = read.i();
        this.invFrames = new float[invFrames_LENGTH];

        for(int INDEX = 0; INDEX < invFrames_LENGTH; ++INDEX) {
            this.invFrames[INDEX] = read.f();
        }
        ...
        this.regenTime = read.f();
        this.resist = read.f();
        this.resistMax = read.f();
        this.resistTime = read.f();
        this.rotation = read.f();
        this.shield = read.f();
        this.spawnedByCore = read.bool();
        this.splitHealthDiv = read.f();
        ...
        this.trueHealth = read.f();
        this.trueMaxHealth = read.f();
        ...
        if (read.bool()) {
            ...
            for(int i = 0; i < seg; ++i) {
                Unit u = (Unit)this.type.constructor.get();
                Wormc w = (Wormc)u;
                current.child(u);
    ...
    public <T extends Unit & Wormc> void distributeActionForward(Cons<T> cons) {
        T current = (T)(this.as());
        cons.get(current);

        while(((Wormc)current).parent() != null) {
            cons.get((Unit)((Wormc)current).parent().as());
            current = (T)(((Wormc)current).parent().as());
        }
    ...
    public void rotateMove(Vec2 vec) {
        ...
        if (!vec.isZero()) {
            this.rotation = Angles.moveToward(this.rotation, vec.angle(), this.type.rotateSpeed * this.speedMultiplier * Math.max(Time.delta, 1.0F));
        }
    ...
    public TextureRegion icon() {
        UnityUnitType uType = (UnityUnitType)this.type;
        if (this.isTail()) {
            return uType.tailOutline;
        } else {
            return !this.isHead() ? uType.segmentOutline : this.type.fullIcon;
        }
    }
    ...
    public Unit addTail() {
        if (!this.isTail()) {
            ...
            Unit tail = (Unit)this.type.constructor.get();
            tail.team = this.team;
            tail.setType(this.type);
            tail.ammo = (float)this.type.ammoCapacity;
            tail.elevation = this.type.flying ? 1.0F : 0.0F;
            tail.heal();
            UnityUnitType uType = (UnityUnitType)this.type;
            if (tail instanceof Wormc) {
                float z = this.layer + 1.0F;
                Tmp.v1.trns(this.rotation() + 180.0F, uType.segmentOffset).add(this);
                tail.set(Tmp.v1);
                ((Wormc)tail).layer(z);
                ((Wormc)tail).head(this.head);
                ((Wormc)tail).parent(this);
                this.child = tail;
    ...
    public void destroy() {
        if (this.trueHealth > 0.0F) {
            this.aggression = 4.0F;
            this.aggressionTime = 600.0F;
            SpecialFx.endDeny.at(this.x, this.y, this.rotation, this);
        } else {
            if (this.isAdded()) {
    ...
    private void updateHealthWorm() {
        if (!this.isHead() && this.head() != null) {
            Endc e = (Endc)this.head().as();
            this.health = this.trueHealth = e.trueHealth();
            this.maxHealth = this.trueMaxHealth = e.trueMaxHealth();
        }
    ...
    public boolean isAI() {
        return this.head != null && !this.isHead() ? this.head.isAI() : this.controller() instanceof AIController;
    }
    ...
    public void add() {
        if (!this.added) {
            ...
            UnityUnitType utype = (UnityUnitType)this.type;
            AntiCheatVariables aType = utype.antiCheatType;
            this.trueHealth = this.type.health;
            this.trueMaxHealth = this.type.health;
            this.trueTeam = this.team;
            Unity.antiCheat.addUnit(this);
            if (aType != null) {
                this.invFrames = new float[aType.invincibilityArray];
            }

            utype = (UnityUnitType)this.type;
            if (this.isHead()) {
                if (this.saveAdd) {
                    for(Unit seg = (Unit)((Wormc)this.child); seg != null; seg = (Unit)((Wormc)((Wormc)seg).child())) {
                        seg.add();
                    }

                    this.saveAdd = false;
                } else {
                    float[] rot = new float[]{this.rotation() + utype.angleLimit};
                    Tmp.v1.trns(rot[0] + 180.0F, utype.segmentOffset + utype.headOffset).add(this);
                    this.distributeActionBack((u) -> {
                        if (u != this) {
                            u.x = Tmp.v1.x;
                            u.y = Tmp.v1.y;
                            ...
                            Tmp.v2.trns(rot[0] + 180.0F, utype.segmentOffset);
                            Tmp.v1.add(Tmp.v2);
                            u.add();
                        }
    ...
    public int cap() {
        int max = Math.max(((UnityUnitType)this.type).maxSegments, ((UnityUnitType)this.type).segmentLength);
        return Units.getCap(this.team) * max;
    }
    ...
    public void write(Writes write) {
        write.f(this.aggression);
        write.f(this.aggressionTime);
        write.f(this.ammo);
        write.i(this.childId);
        TypeIO.writeController(write, this.controller);
        write.f(this.elevation);
        write.d(this.flag);
        write.i(this.headId);
        write.f(this.health);
        write.i(this.invFrames.length);

        for(int INDEX = 0; INDEX < this.invFrames.length; ++INDEX) {
            write.f(this.invFrames[INDEX]);
        }
        ...
        write.f(this.regenTime);
        write.f(this.resist);
        write.f(this.resistMax);
        write.f(this.resistTime);
        write.f(this.rotation);
        write.f(this.shield);
        write.bool(this.spawnedByCore);
        write.f(this.splitHealthDiv);
        ...
        write.f(this.trueHealth);
        write.f(this.trueMaxHealth);
        write.s(this.type.id);
        write.bool(this.updateBuilding);
        TypeIO.writeVec2(write, this.vel);
        write.f(this.waitTime);
        write.f(this.x);
        write.f(this.y);
        write.bool(this.isHead());
        if (this.isHead()) {
            Wormc ch = (Wormc)this.child;

            int amount;
            for(amount = 0; ch != null; ch = (Wormc)ch.child()) {
                ++amount;
            }

            write.s(amount);

            for(Wormc var7 = (Wormc)this.child; var7 != null; var7 = (Wormc)var7.child()) {
                write.b(this.weaponIdx);
                var7.write(write);
            }
        }
            ...
            if (this.added) {
                ...
                if (uType.splittable) {
                    if (this.child != null && this.parent != null) {
                        uType.splitSound.at(this.x(), this.y());
                    }

                    if (this.child != null) {
                        Unit wc = (Unit)((Wormc)this.child);

                        for(float z = 0.0F; wc != null; wc = (Unit)((Wormc)((Wormc)wc).child())) {
                            ((Wormc)wc).layer(z++);
                            ((Wormc)wc).splitHealthDiv(((Wormc)wc).splitHealthDiv() * 2.0F);
                            ((Wormc)wc).head(this.child);
                            if (((Wormc)wc).isTail()) {
                                ((Wormc)wc).waitTime(300.0F);
                            }
                        }
                    }

                    if (this.parent != null) {
                        Wormc wp = (Wormc)this.parent;
                        this.distributeActionForward((u) -> {
                            if (u != this) {
                                ((Wormc)u).splitHealthDiv(((Wormc)u).splitHealthDiv() * 2.0F);
                            }
                ...
                if (!this.isHead() && !uType.splittable && !this.removing) {
                    this.head.remove();
                } else {
                    if (this.isHead() && !uType.splittable) {
                        this.distributeActionBack((u) -> {
                            if (u != this) {
                                ((Wormc)u).removing(true);
                                u.remove();
                                ((Wormc)u).removing(false);
                            }
    ...
    public void update() {
        this.updateHealthWorm();
        UnityUnitType utype = (UnityUnitType)this.type;
        AntiCheatVariables aType = utype.antiCheatType;
        if (aType != null) {
            if (this.team != this.trueTeam && this.trueHealth >= Math.max(this.trueMaxHealth / 100.0F, 150.0F)) {
                this.team = this.trueTeam;
            }

            if (this.health < this.trueHealth || Float.isNaN(this.health)) {
                this.health = this.trueHealth;
            }

            this.trueHealth = this.health;
            if (this.maxHealth < this.trueMaxHealth || Float.isNaN(this.maxHealth)) {
                this.maxHealth = this.trueMaxHealth;
            }

            this.trueMaxHealth = this.maxHealth;
            if (this.trueHealth > 0.0F) {
                this.dead = false;
            }
            ...
            for(int i = 0; i < this.invFrames.length; ++i) {
                this.invFrames[i] = Math.max(this.invFrames[i] - Time.delta, 0.0F);
            }
        }

        if (this.invTimer > 0.0F) {
            this.invTimer -= Time.delta;
        }

        if (this.aggression > 0.0F) {
            for(WeaponMount mount : this.mounts) {
                mount.reload = Math.max(0.0F, mount.reload - this.aggression * Time.delta);
            }

            if (this.aggressionTime > 0.0F) {
                this.aggressionTime -= Time.delta;
            } else {
                this.aggression = Mathf.lerpDelta(this.aggression, 0.0F, 0.1F);
            }
        }
        ...
        utype = (UnityUnitType)this.type;
        if (utype.splittable && this.isTail() && utype.regenTime > 0.0F) {
            int forward = this.countFoward();
            if (forward < Math.max(utype.maxSegments, utype.segmentLength)) {
                this.regenTime += Time.delta;
                if (this.regenTime >= utype.regenTime) {
                    this.regenTime = 0.0F;
                    Unit unit;
                    if ((unit = this.addTail()) != null) {
                        this.health /= 2.0F;
        ...
        if (this.isTail() && this.waitTime > 0.0F) {
            this.waitTime -= Time.delta;
        }

        if (!utype.splittable) {
            if (!this.isHead()) {
                this.health = this.head.health;
            }

            if (this.isHead() && this.isAdded() || this.head != null && this.head.isAdded()) {
                for(Wormc t = (Wormc)this.child; t != null && !t.isAdded(); t = (Wormc)t.child()) {
                    t.add();
                }
            }
        }

        if (utype.splittable && (this.parent != null || this.child != null) && this.dead) {
            this.destroy();
        }
    ...
    public void heal(float amount) {
        this.health += Math.max(0.0F, amount);
        this.trueHealth = this.health;
        this.clampHealth();
    }
    ...
    public void afterSync() {
        if (this.headId != -1 && this.head == null) {
            Unit h = (Unit)Groups.unit.getByID(this.headId);
            if (h instanceof Wormc) {
                Wormc wc = (Wormc)h;
                this.head = h;
                this.headId = -1;
            }
        ...
        if (this.childId != -1 && this.child == null) {
            Unit c = (Unit)Groups.unit.getByID(this.childId);
            if (c instanceof Wormc) {
                Wormc wc = (Wormc)c;
                this.child = c;
                wc.parent(this);
                this.childId = -1;
    ...
    public void setupWeapons(UnitType def) {
        ...
        if (!this.isHead()) {
            Seq<Weapon> seq = uType.segmentWeapons[this.weaponIdx];
            this.mounts = new WeaponMount[seq.size];

            for(int i = 0; i < this.mounts.length; ++i) {
                this.mounts[i] = (WeaponMount)((Weapon)seq.get(i)).mountType.get((Weapon)seq.get(i));
            }
    ...
    public void updateLaserSpeed() {
        if (this.isHead() && this.mounts != null && this.mounts[0].bullet != null) {
            this.speedMultiplier *= 0.075F;
        }
    ...
    public boolean isHead() {
        return this.parent == null || this.head == null || this.head == this;
    }
    ...
    public <T extends Unit & Wormc> void distributeActionBack(Cons<T> cons) {
        T current = (T)(this.as());
        cons.get(current);

        while(((Wormc)current).child() != null) {
            cons.get((Unit)((Wormc)current).child().as());
            current = (T)(((Wormc)current).child().as());
        }
    ...
    private void updatePost() {
        if (this.isHead()) {
            ...
            this.distributeActionBack((u) -> {
                if (u != this) {
                    float offset = this == last ? uType.headOffset : 0.0F;
                    Tmp.v1.trns(last.rotation + 180.0F, uType.segmentOffset / 2.0F + offset).add(last);
                    float rdx = u.deltaX - last.deltaX;
                    float rdy = u.deltaY - last.deltaY;
                    float angTo = uType.preventDrifting && (!(last.deltaLen() > 0.001F) || !(rdx * rdx + rdy * rdy > 1.0E-5F)) ? u.rotation : u.angleTo(Tmp.v1);
                    u.rotation = angTo - Utils.angleDistSigned(angTo, last.rotation, uType.angleLimit) * (1.0F - uType.anglePhysicsSmooth);
                    u.trns(Tmp.v3.trns(u.rotation, last.deltaLen()));
                    Tmp.v2.trns(u.rotation, uType.segmentOffset / 2.0F).add(u);
                    Tmp.v2.sub(Tmp.v1).scl(Mathf.clamp(uType.jointStrength * Time.delta));
                    Unit n = u;

                    for(int cast = uType.segmentCast; cast > 0 && n != null; --cast) {
                        float scl = (float)cast / (float)uType.segmentCast;
                        n.set(n.x - Tmp.v2.x * scl, n.y - Tmp.v2.y * scl);
                        n.updateLastPosition();
                        n = ((Wormc)n).child();
                    }

                    float nextHealth = (last.health() + u.health()) / 2.0F;
                    if (!Mathf.equal(nextHealth, last.health(), 1.0E-4F)) {
                        last.health(Mathf.lerpDelta(last.health(), nextHealth, uType.healthDistribution));
                    }

                    if (!Mathf.equal(nextHealth, u.health(), 1.0E-4F)) {
                        u.health(Mathf.lerpDelta(u.health(), nextHealth, uType.healthDistribution));
                    }

                    Wormc wrm = (Wormc)last;
                    float nextHealthDv = (wrm.splitHealthDiv() + ((Wormc)u).splitHealthDiv()) / 2.0F;
                    if (!Mathf.equal(nextHealth, wrm.splitHealthDiv(), 1.0E-4F)) {
                        wrm.splitHealthDiv(Mathf.lerpDelta(wrm.splitHealthDiv(), nextHealthDv, uType.healthDistribution));
                    }

                    if (!Mathf.equal(nextHealth, ((Wormc)u).splitHealthDiv(), 1.0E-4F)) {
                        ((Wormc)u).splitHealthDiv(Mathf.lerpDelta(((Wormc)u).splitHealthDiv(), nextHealthDv, uType.healthDistribution));
                    }

                    last = u;
                }
            });
            this.scanTime += Time.delta;
            if (this.scanTime >= 5.0F && uType.chainable) {
                Tmp.v1.trns(this.rotation(), uType.segmentOffset / 2.0F).add(this);
                Tmp.r1.setCentered(Tmp.v1.x, Tmp.v1.y, this.hitSize());
                Units.nearby(Tmp.r1, (u) -> {
                    if (u.team == this.team && u.type == this.type && u instanceof Wormc) {
                        Wormc w = (Wormc)u;
                        if (w.head() != this && w.isTail() && w.countFoward() + this.countBackward() < uType.maxSegments && w.waitTime() <= 0.0F && this.within(u, uType.segmentOffset) && Utils.angleDist(this.rotation(), this.angleTo(u)) < uType.angleLimit) {
                            this.connect(w);
                        }
                    }
    ...
    private void updateHealthDiv() {
        this.healthMultiplier /= this.splitHealthDiv;
    }
    ...
    private void removeWorm() {
        UnityUnitType utype = (UnityUnitType)this.type;
        if (this.isAdded() && !utype.splittable && !this.isHead() && this.head() != null && !this.head().isAdded() && ((Endc)this.head()).trueHealth() <= 0.0F) {
            this.trueHealth = 0.0F;
            this.health = 0.0F;
        }
    ...
    public void controller(UnitController next) {
        if (next instanceof Player && this.head != null && !this.isHead()) {
            this.head.controller(next);
        } else {
            this.clearCommand();
    ...
    public void impulse(float x, float y) {
        ...
        Tmp.v1.set(x, y).scl(scl).limit(mass * 8.0F);
        if (!Tmp.v1.isNaN()) {
            x = Tmp.v1.x;
            y = Tmp.v1.y;
        } else {
        ...
        if (len > mass * 8.0F) {
            float l = (len - mass * 8.0F) / (mass / 2.0F);
            this.resist += l;
            this.resistMax = Math.max(this.resistMax, this.resist);
            this.resistTime = Math.max(this.resistTime, a.resistTime / 2.0F);
            SpecialFx.endDeny.at(this.x, this.y, this.rotation, Tmp.c1.a(Mathf.clamp(l / (mass * 15.0F))), this);
        }
    ...
    public static Oppression create() {
        return new Oppression();
    }

    public int classId() {
        return UnityEntityMapping.classId(Oppression.class);
    }
    ...
    public void parent(Unit parent) {
        this.parent = parent;
    }
    ...
    public void scanTime(float scanTime) {
    ...
    public void weaponIdx(byte weaponIdx) {
    ...
    public void saveAdd(boolean saveAdd) {
        this.saveAdd = saveAdd;
    }

    public float splitHealthDiv() {
        return this.splitHealthDiv;
    }

    public void splitHealthDiv(float splitHealthDiv) {
        this.splitHealthDiv = splitHealthDiv;
    }
    ...
    public void regenTime(float regenTime) {
    ...
    public void childId(int childId) {
