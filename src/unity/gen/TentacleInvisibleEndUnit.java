...
import unity.entities.NewTentacle;
...
public class TentacleInvisibleEndUnit extends Unit implements Drawc, Hitboxc, Itemsc, Tentaclec, Commanderc, Endc, Weaponsc, Healthc, Builderc, Velc, Statusc, Entityc, Physicsc, Flyingc, Posc, Rotc, Shieldc, Invisiblec, Unintersectablec, Teamc, Unitc, Factionc, Boundedc, Minerc, Syncc {
    ...
    public transient Seq<NewTentacle> tentacles = new Seq();
    ...
    protected transient float disabledTime;
    protected transient Interval scanInterval;
    public float alphaLerp;
    private transient float alphaLerp_TARGET_;
    private transient float alphaLerp_LAST_;
    private UnitController controller;
    private transient float resupplyTime;
    private transient boolean wasPlayer;
    private transient boolean wasHealed;

    protected TentacleInvisibleEndUnit() {
        this.applied = new Bits(Vars.content.getBy(ContentType.status).size);
        this.disabledTime = 120.0F;
        this.scanInterval = new Interval(2);
        this.resupplyTime = Mathf.random(10.0F);
    }

    public String toString() {
        return "TentacleInvisibleEndUnit#" + this.id;
    }
    ...
    public void damage(float amount) {
        ...
        this.disabledTime = Math.max(this.disabledTime, Math.max(84.0F, amount / 25.0F));
    }
    ...
    public void snapInterpolation() {
        ...
        this.alphaLerp_LAST_ = this.alphaLerp;
        this.alphaLerp_TARGET_ = this.alphaLerp;
    ...
    public void snapSync() {
        this.updateSpacing = 16L;
        this.lastUpdated = Time.millis();
        this.alphaLerp_LAST_ = this.alphaLerp_TARGET_;
        this.alphaLerp = this.alphaLerp_TARGET_;
    ...
    public void readSyncManual(FloatBuffer buffer) {
        ...
        this.lastUpdated = Time.millis();
        this.alphaLerp_LAST_ = this.alphaLerp;
        this.alphaLerp_TARGET_ = buffer.get();
        this.rotation_LAST_ = this.rotation;
    ...
    public void add() {
        if (!this.added) {
            ...
            this.updateLastPosition();
            UnityUnitType uType = (UnityUnitType)this.type;
            if (this.tentacles.isEmpty() && !uType.tentacles.isEmpty()) {
                for(TentacleType tentacle : uType.tentacles) {
                    this.tentacles.add(new NewTentacle(tentacle, this));
                }
            }
    ...
    public void interpolate() {
        if (this.lastUpdated != 0L && this.updateSpacing != 0L) {
            ...
            this.alphaLerp = Mathf.lerp(this.alphaLerp_LAST_, this.alphaLerp_TARGET_, alpha);
    ...
    public boolean isValid() {
        return !this.dead && this.added && this.intersects();
    }
    ...
    public void hitbox(Rect rect) {
        if (!this.intersects()) {
            ...
            if (caller != null && QuadTree.class.isAssignableFrom(caller)) {
                rect.set(this.x, this.y, Float.NaN, Float.NaN);
                return;
            }
        }
    ...
    public void update() {
        Entityc var2 = this.self();
        if (var2 instanceof Physicsc) {
            Physicsc e = (Physicsc)var2;
            if (e.physref() != null) {
                if (this.intersects()) {
                    if (e.physref().body.radius < 0.0F) {
                        e.physref().body.radius = 0.0F;
                    }

                    e.physref().body.radius = Mathf.lerpDelta(e.physref().body.radius, this.hitSize() / 2.0F, 0.2F);
                } else {
        ...
        this.tentacles.each(NewTentacle::update);
        ...
        if (this.scanInterval.get(0, 5.0F) && this.invisible) {
            this.hitbox(Tmp.r1);
        ...
        this.invisible = this.alphaLerp >= 0.5F;
    ...
    public void drawTentacles() {
        this.tentacles.each(NewTentacle::draw);
    }
    ...
    public static TentacleInvisibleEndUnit create() {
        return new TentacleInvisibleEndUnit();
    }

    public int classId() {
        return UnityEntityMapping.classId(TentacleInvisibleEndUnit.class);
    }

    public Seq<NewTentacle> tentacles() {
        return this.tentacles;
    }

    public void tentacles(Seq<NewTentacle> tentacles) {
    ...
    public Interval scanInterval() {
        return this.scanInterval;
    }

    public float alphaLerp() {
        return this.alphaLerp;
    }

    public void alphaLerp(float alphaLerp) {
