...
public class TransLegsUnit extends Unit implements Drawc, Hitboxc, Itemsc, Legsc, Commanderc, Weaponsc, Healthc, Builderc, Velc, Statusc, Entityc, Physicsc, Flyingc, Posc, Rotc, Shieldc, Teamc, Unitc, Transc, Boundedc, Minerc, Syncc {
    ...
    public void update() {
        ...
        if (type.transPred.get(this)) {
            if (!(this.transformTime < 0.0F) && !(this.transformTime > type.transformTime)) {
                this.transformTime -= Time.delta;
            } else {
                Unit next = ((UnitType)type.toTrans.get(this)).spawn(this.team, this.x, this.y);
                next.rotation = this.rotation;
                next.add();
                next.vel.set(this.vel);
                if (this.isPlayer()) {
                    next.controller(this.controller());
                }
    ...
    public void setType(UnitType type) {
        ...
        if (type instanceof UnityUnitType) {
            ...
            this.transformTime = def.transformTime;
        }
    ...
    public static TransLegsUnit create() {
        return new TransLegsUnit();
    }
    ...
    public void transformTime(float transformTime) {
