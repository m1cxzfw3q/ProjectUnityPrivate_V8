...
import unity.util.TriJointInverseKinematics;

public class TriJointLegsUnit extends Unit implements Statusc, Entityc, Drawc, Hitboxc, Physicsc, Itemsc, Flyingc, Shieldc, Posc, Commanderc, Syncc, TriJointLegsc, Weaponsc, Teamc, Unitc, Healthc, Rotc, Boundedc, Builderc, Minerc, Velc {
    ...
    public transient TriJointLeg[] legs;
    ...
    public String toString() {
        return "TriJointLegsUnit#" + this.id;
    }
    ...
    private void resetlegs() {
        ...
        this.legs = new TriJointLeg[count];

        for(int i = 0; i < this.legs.length; ++i) {
            TriJointLeg l = new TriJointLeg();

            for(int j = 0; j < 3; ++j) {
                l.joints[j].trns((float)i * spacing + rot, legLength / 3.0F * ((float)j + 1.0F)).add(this);
            }
    ...
    public void update() {
        ...
        if (Mathf.dst(this.deltaX, this.deltaY) > 0.001F) {
            this.baseRotation = Angles.moveToward(this.baseRotation, Mathf.angle(this.deltaX, this.deltaY), this.type.baseRotateSpeed);
        }

        float rot = this.baseRotation;
        float legLength = this.type.legLength;
        if (this.legs == null || this.legs.length != this.type.legCount) {
            this.resetlegs();
        }

        int div = Math.max(this.legs.length / this.type.legGroupSize, 2);
        this.moveSpace = legLength / 2.4F / ((float)div / 2.0F) * this.type.legMoveSpace;
        ...
        for(int i = 0; i < this.legs.length; ++i) {
            ...
            Vec2 baseOffset = Tmp.v5.trns(dstRot, this.type.legBaseOffset).add(this);
            TriJointLeg l = this.legs[i];
            ...
            if (l.group != group) {
                if (!move && i % div == l.group) {
                    ...
                    if (floor.isLiquid) {
                        floor.walkEffect.at(l.joints[2].x, l.joints[2].y, this.type.rippleScale, floor.mapColor);
                        floor.walkSound.at(this.x, this.y, 1.0F, floor.walkSoundVolume);
                    } else {
                        Fx.unitLandSmall.at(l.joints[2].x, l.joints[2].y, this.type.rippleScale, floor.mapColor);
                    }

                    if (this.type.landShake > 0.0F) {
                        Effect.shake(this.type.landShake, this.type.landShake, l.joints[2]);
                    }

                    if (this.type.legSplashDamage > 0.0F) {
                        Damage.damage(this.team, l.joints[2].x, l.joints[2].y, this.type.legSplashRange, this.type.legSplashDamage, false, true);
                    }
                }
            ...
            if (move) {
                ...
                Tmp.v1.trns(dstRot, legLength).add(baseOffset).add(moveOffset, backLeg ? 1.5F : 1.0F);
            ...
            if (!baseOffset.within(legDest, legLength)) {
                float scl = Math.min(baseOffset.dst(legDest), this.type.legLength * this.type.maxStretch) / legLength - 1.0F;
                if (move) {
                    float moveFract = stageF % 1.0F;
                    l.legScl = Mathf.lerpDelta(l.legScl, 1.0F + scl, moveFract);
                    l.jointLerp = this.type.legSpeed / 4.0F;
                } else {
                    l.legScl = 1.0F + scl;
                    l.jointLerp = Mathf.lerpDelta(l.jointLerp, 1.0F, this.type.legSpeed / 2.0F);
                }
            } else {
                l.legScl = Mathf.lerpDelta(l.legScl, 1.0F, this.type.legSpeed / 4.0F);
                l.jointLerp = this.type.legSpeed / 4.0F;
            }

            TriJointInverseKinematics.solve(baseOffset.x, baseOffset.y, l.joints, this.type.legLength / 3.0F * l.legScl, legDest, side, move ? 1.0F : (baseOffset.within(legDest, legLength) ? 1.0F : l.jointLerp * Time.delta));
        }
        ...
        if (this.team != Vars.state.rules.waveTeam && Vars.state.hasSpawns() && (!Vars.net.client() || this.isLocal())) {
            ...
            for(Tile spawn : Vars.spawner.getSpawns()) {
                if (this.within(spawn.worldx(), spawn.worldy(), rot)) {
                    this.velAddNet(Tmp.v1.set(this).sub(spawn.worldx(), spawn.worldy()).setLength(1.1F - this.dst(spawn) / rot).scl(0.45F * Time.delta));
                }
            }
        }
    ...
    public static TriJointLegsUnit create() {
        return new TriJointLegsUnit();
    }

    public int classId() {
        return UnityEntityMapping.classId(TriJointLegsUnit.class);
    }
    ...
    public void legs(TriJointLeg[] legs) {
        this.legs = legs;
    }
