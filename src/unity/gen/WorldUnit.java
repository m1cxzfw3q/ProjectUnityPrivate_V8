...
import unity.entities.UnitTile;
...
public class WorldUnit extends Unit implements Statusc, Entityc, Drawc, Hitboxc, Physicsc, Itemsc, Flyingc, Shieldc, Posc, Commanderc, Syncc, Worldc, Weaponsc, Teamc, Unitc, Healthc, Rotc, Boundedc, Builderc, Minerc, Velc {
    ...
    public transient Seq<Time.DelayRun> runs;
    public transient World unitWorld;
    public transient Seq<Building> buildings;
    public transient Seq<Turret.TurretBuild> turrets;
    public transient FloatSeq positions;
    ...
    public void setup() {
        ...
        int w = uType.worldWidth;
        int h = uType.worldHeight;
        this.unitWorld = new World();
        this.unitWorld.tiles = new Tiles(w, h);

        for(int i = 0; i < w * h; ++i) {
            this.unitWorld.tiles.set(i % w, i / w, new UnitTile(i % w, i / w));
        }

        this.unitWorld.tiles.eachTile((tile) -> tile.setFloor(Blocks.metalFloor.asFloor()));
        tmp.clear();
        tmpr.clear();
        ...
        for(Building building : tmp) {
            if (this.validPlace(building.tile) && tmpAdded.add(building.id)) {
                int tx = this.conX(building.tile.x);
                int ty = this.conY(building.tile.y);
                if (building.power != null && building instanceof PowerNode.PowerNodeBuild && !building.power.links.isEmpty()) {
                    IntSeq seq = building.power.links;
                    IntSeq nseq = new IntSeq();

                    for(int i = 0; i < seq.size; ++i) {
                        int pos = seq.get(i);
                        int cx = this.conX(Point2.x(pos));
                        int cy = this.conY(Point2.y(pos));
                        if (this.valid(cx, cy)) {
                ...
                tmpr.add((Runnable)() -> {
                    building.x = this.cwX(building.x);
                    building.y = this.cwY(building.y);
                    this.unitWorld.tile(tx, ty).setBlock(building.block, building.team, building.rotation, () -> building);
                });
        ...
        for(Building b : this.buildings) {
            if (b instanceof PowerNode.PowerNodeBuild) {
                IntSeq seq = (IntSeq)tmpLinks.get(b.id);
    ...
    public boolean valid(int x, int y) {
        return x >= 0 && x < this.unitWorld.width() && y >= 0 && y < this.unitWorld.height();
    }
    ...
    public void update() {
        ...
        float cx = (float)(this.unitWorld.width() * 8) / 2.0F;
        float cy = (float)(this.unitWorld.height() * 8) / 2.0F;
        float r = this.rotation - 90.0F;
        this.positions.clear();
        TimeReflect.swapRuns(this.runs);
        World ow = Vars.world;
        Vars.world = this.unitWorld;
        if (this.isPlayer()) {
            for(Turret.TurretBuild t : this.turrets) {
                t.logicControlTime = 5.0F;
                t.logicShooting = this.isShooting();
                t.targetPos.set(this.aimX(), this.aimY());
            }
        }
    ...
    public int conY(int y) {
        return y - (int)(this.y / 8.0F) + this.unitWorld.height() / 2 - 1;
    }
    ...
    public float cwX(float x) {
        return x - this.x + (float)(this.unitWorld.width() * 8) / 2.0F;
    }
    ...
    public boolean validPlace(Tile tile) {
        ...
        int tx = this.conX(tile.x);
        int ty = this.conY(tile.y);
        boolean valid = tx >= 0 && tx < this.unitWorld.width() && ty >= 0 && ty < this.unitWorld.height();
        if (tile.block().isMultiblock()) {
            for(int dx = 0; dx < block.size; ++dx) {
                for(int dy = 0; dy < block.size; ++dy) {
                    int wx = dx + offset + tx;
                    int wy = dy + offset + ty;
                    valid &= wx >= 0 && wx < this.unitWorld.width() && wy >= 0 && wy < this.unitWorld.height();
                }
            }
        }
    ...
    public float cwY(float y) {
        return y - this.y + (float)(this.unitWorld.height() * 8) / 2.0F;
    }
    ...
    public int conX(int x) {
        return x - (int)(this.x / 8.0F) + this.unitWorld.width() / 2 - 1;
    }
    ...
    public static WorldUnit create() {
        return new WorldUnit();
    }

    public int classId() {
        return UnityEntityMapping.classId(WorldUnit.class);
    }

    public Seq<Time.DelayRun> runs() {
        return this.runs;
    }

    public void runs(Seq<Time.DelayRun> runs) {
        this.runs = runs;
    }

    public World unitWorld() {
        return this.unitWorld;
    }

    public void unitWorld(World unitWorld) {
    ...
    public void buildings(Seq<Building> buildings) {
        this.buildings = buildings;
    }

    public Seq<Turret.TurretBuild> turrets() {
        return this.turrets;
    }

    public void turrets(Seq<Turret.TurretBuild> turrets) {
        this.turrets = turrets;
    }

    public FloatSeq positions() {
        return this.positions;
    }

    public void positions(FloatSeq positions) {
