...
public class UnityDrawf {
    ...
    private static final Vec2 AB = new Vec2();
    private static final Vec2 B = new Vec2();
    private static final Vec2 BC = new Vec2();
    private static final Vec2 C = new Vec2();
    private static final Vec2 D = new Vec2();
    private static final Vec2 D0 = new Vec2();
    private static final Vec2 E = new Vec2();
    private static final Vec2 E0 = new Vec2();
    private static final Vec3 q1 = new Vec3();
    private static final Vec3 q2 = new Vec3();
    private static final Vec3 q3 = new Vec3();
    private static final Vec3 q4 = new Vec3();
    private static final Vec3 v31 = new Vec3();
    private static final Mat3D m41 = new Mat3D();
    public static final byte[] tileMap = new byte[]{39, 36, 39, 36, 27, 16, 27, 24, 39, 36, 39, 36, 27, 16, 27, 24, 38, 37, 38, 37, 17, 41, 17, 43, 38, 37, 38, 37, 26, 21, 26, 25, 39, 36, 39, 36, 27, 16, 27, 24, 39, 36, 39, 36, 27, 16, 27, 24, 38, 37, 38, 37, 17, 41, 17, 43, 38, 37, 38, 37, 26, 21, 26, 25, 3, 4, 3, 4, 15, 40, 15, 20, 3, 4, 3, 4, 15, 40, 15, 20, 5, 28, 5, 28, 29, 10, 29, 23, 5, 28, 5, 28, 31, 11, 31, 32, 3, 4, 3, 4, 15, 40, 15, 20, 3, 4, 3, 4, 15, 40, 15, 20, 2, 30, 2, 30, 9, 47, 9, 22, 2, 30, 2, 30, 14, 44, 14, 6, 39, 36, 39, 36, 27, 16, 27, 24, 39, 36, 39, 36, 27, 16, 27, 24, 38, 37, 38, 37, 17, 41, 17, 43, 38, 37, 38, 37, 26, 21, 26, 25, 39, 36, 39, 36, 27, 16, 27, 24, 39, 36, 39, 36, 27, 16, 27, 24, 38, 37, 38, 37, 17, 41, 17, 43, 38, 37, 38, 37, 26, 21, 26, 25, 3, 0, 3, 0, 15, 42, 15, 12, 3, 0, 3, 0, 15, 42, 15, 12, 5, 8, 5, 8, 29, 35, 29, 33, 5, 8, 5, 8, 31, 34, 31, 7, 3, 0, 3, 0, 15, 42, 15, 12, 3, 0, 3, 0, 15, 42, 15, 12, 2, 1, 2, 1, 9, 45, 9, 19, 2, 1, 2, 1, 14, 18, 14, 13};
    ...
    private static void preparePointyJoin(Vec2 A, Vec2 B, Vec2 C, Vec2 D, Vec2 E, float halfLineWidth) {
        ...
        if (!Mathf.equal(angle, 0.0F) && !Mathf.equal(angle, ((float)Math.PI * 2F))) {
            float len = (float)((double)halfLineWidth / Math.sin((double)angle));
    ...
    private static float angleRad() {
        return (float)Math.atan2((double)(BC.x * AB.y - BC.y * AB.x), (double)(AB.x * BC.x + AB.y * BC.y));
    }
    ...
    public static void dashCircleAngle(float x, float y, float radius, float rotation) {
        ...
        for(int i = 0; i < sides; ++i) {
            if (i % 2 != 0) {
                vec1.set(radius, 0.0F).setAngle(360.0F / (float)sides * (float)i + 90.0F + rotation);
                float x1 = vec1.x;
                float y1 = vec1.y;
                vec1.set(radius, 0.0F).setAngle(360.0F / (float)sides * (float)(i + 1) + 90.0F + rotation);
    ...
    public static void diamond(float x, float y, float width, float length, float backLengthScl, float rotation) {
        float tx1 = Angles.trnsx(rotation + 90.0F, width);
        float ty1 = Angles.trnsy(rotation + 90.0F, width);
        float tx2 = Angles.trnsx(rotation, length);
        float ty2 = Angles.trnsy(rotation, length);
        Fill.quad(x + tx1, y + ty1, x + tx2, y + ty2, x - tx1, y - ty1, x - tx2 * backLengthScl, y - ty2 * backLengthScl);
    }
    ...
    public static void diamond(float x, float y, float width, float length, float rotation) {
        ...
        float tx2 = Angles.trnsx(rotation, length);
        float ty2 = Angles.trnsy(rotation, length);
        Fill.quad(x + tx1, y + ty1, x + tx2, y + ty2, x - tx1, y - ty1, x - tx2, y - ty2);
    }
    ...
    public static void shiningCircle(int seed, float time, float x, float y, float radius, int spikes, float spikeDuration, float spikeWidth, float spikeHeight) {
        shiningCircle(seed, time, x, y, radius, spikes, spikeDuration, spikeWidth, spikeHeight, 0.0F);
    }
    ...
    public static void shiningCircle(int seed, float time, float x, float y, float radius, int spikes, float spikeDuration, float spikeWidth, float spikeHeight, float angleDrift) {
        shiningCircle(seed, time, x, y, radius, spikes, spikeDuration, 0.0F, spikeWidth, spikeHeight, angleDrift);
    }
    ...
    public static void shiningCircle(int seed, float time, float x, float y, float radius, int spikes, float spikeDuration, float durationRange, float spikeWidth, float spikeHeight, float angleDrift) {
        ...
        spikeWidth = Math.min(spikeWidth, 90.0F);

        for(int i = 0; i < spikes; ++i) {
            float d = spikeDuration * (durationRange > 0.0F ? Mathf.randomSeed((long)(seed + i) * 41L, 1.0F - durationRange, 1.0F + durationRange) : 1.0F);
            float timeOffset = Mathf.randomSeed((long)(seed + i) * 314L, 0.0F, d);
            int timeSeed = Mathf.floor((time + timeOffset) / d);
            float fin = (time + timeOffset) % d / d;
            float fslope = (0.5F - Math.abs(fin - 0.5F)) * 2.0F;
            float angle = Mathf.randomSeed((long)Math.max(timeSeed, 1) + (long)(i + seed) * 245L, 360.0F);
            if (fslope > 1.0E-4F) {
                int idx = 0;
                float drift = angleDrift > 0.0F ? Mathf.randomSeed((long)Math.max(timeSeed, 1) + (long)(i + seed) * 162L, -angleDrift, angleDrift) * fin : 0.0F;

                for(int j = 0; j < 3; ++j) {
                    float angB = (float)j * spikeWidth - 2.0F * spikeWidth / 2.0F + angle;
                    Tmp.v1.trns(angB + drift, radius + (j == 1 ? spikeHeight * fslope : 0.0F)).add(x, y);
    ...
    public static void panningCircle(TextureRegion region, float x, float y, float w, float h, float radius, float arcCone, float arcRotation, Quat rotation, float layerLow, float layerHigh) {
        panningCircle(region, x, y, w, h, radius, arcCone, arcRotation, rotation, false, layerLow, layerHigh, 150.0F);
    }
    ...
    public static void panningCircle(TextureRegion region, float x, float y, float w, float h, float radius, float arcCone, float arcRotation, Quat rotation, boolean useLinePrecision, float layerLow, float layerHigh) {
        panningCircle(region, x, y, w, h, radius, arcCone, arcRotation, rotation, useLinePrecision, layerLow, layerHigh, 150.0F);
    }
    ...
    public static void panningCircle(TextureRegion region, float x, float y, float w, float h, float radius, float arcCone, float arcRotation, Quat rotation, boolean useLinePrecision, float layerLow, float layerHigh, float perspectiveDst) {
        float z = Draw.z();
        float arc = arcCone / 360.0F;
        int sides = useLinePrecision ? (int)((float)Lines.circleVertices(radius) * arc) : (int)(((float)Math.PI * 2F) * radius * arc / w);
        float space = arcCone / (float)sides;
        float hstep = Lines.getStroke() * h / 2.0F / Mathf.cosDeg(space / 2.0F);
        float r1 = radius - hstep;
        float r2 = radius + hstep;

        for(int i = 0; i < sides; ++i) {
            float a = arcRotation - arcCone / 2.0F + space * (float)i;
            ...
            Mat3D.rot(v31.set(r1 * cos, r1 * sin, 0.0F), m41).scl(Math.max((perspectiveDst + v31.z) / perspectiveDst, 0.0F));
            float x1 = x + v31.x;
            float y1 = y + v31.y;
            float sumZ = v31.z;
            Mat3D.rot(v31.set(r1 * cos2, r1 * sin2, 0.0F), m41).scl(Math.max((perspectiveDst + v31.z) / perspectiveDst, 0.0F));
            float x2 = x + v31.x;
            float y2 = y + v31.y;
            sumZ += v31.z;
            Mat3D.rot(v31.set(r2 * cos2, r2 * sin2, 0.0F), m41).scl(Math.max((perspectiveDst + v31.z) / perspectiveDst, 0.0F));
            float x3 = x + v31.x;
            float y3 = y + v31.y;
            sumZ += v31.z;
            Mat3D.rot(v31.set(r2 * cos, r2 * sin, 0.0F), m41).scl(Math.max((perspectiveDst + v31.z) / perspectiveDst, 0.0F));
            float x4 = x + v31.x;
            float y4 = y + v31.y;
            sumZ = (sumZ + v31.z) / 4.0F;
            Draw.z(sumZ >= 0.0F ? layerHigh : layerLow);
    ...
    public static void quad(Texture texture, float x1, float y1, float u1, float v1, float x2, float y2, float u2, float v2, float x3, float y3, float u3, float v3, float x4, float y4, float u4, float v4) {
        ...
        float mix = Draw.getMixColor().toFloatBits();
        vertices[0] = x1;
        vertices[1] = y1;
        vertices[2] = col;
        vertices[3] = u1;
    ...
    public static void tri(float x, float y, float width, float length, float rotation) {
        float ax = Angles.trnsx(rotation - 90.0F, width / 2.0F);
        float ay = Angles.trnsy(rotation - 90.0F, width / 2.0F);
        float tx = Angles.trnsx(rotation, length);
        float ty = Angles.trnsy(rotation, length);
        Fill.tri(x + ax, y + ay, x + tx, y + ty, x - ax, y - ay);
    }
    ...
    public static void arcLine(float x, float y, float radius, float arcAngle, float angle) {
        float arc = arcAngle / 360.0F;
        int sides = (int)((float)Lines.circleVertices(radius) * arc);
        float space = arcAngle / (float)sides;
        float hstep = Lines.getStroke() / 2.0F / Mathf.cosDeg(space / 2.0F);
        float r1 = radius - hstep;
        float r2 = radius + hstep;

        for(int i = 0; i < sides; ++i) {
            float a = angle - arcAngle / 2.0F + space * (float)i;
            float cos = Mathf.cosDeg(a);
            float sin = Mathf.sinDeg(a);
            float cos2 = Mathf.cosDeg(a + space);
            float sin2 = Mathf.sinDeg(a + space);
    ...
    public static void arcFill(float x, float y, float radius, float arcAngle, float angle) {
        ...
        for(int i = 0; i < sides; ++i) {
            ...
            float cos2 = Mathf.cosDeg(a + space);
            float sin2 = Mathf.sinDeg(a + space);
            Fill.tri(x, y, x + radius * cos, y + radius * sin, x + radius * cos2, y + radius * sin2);
        }
    ...
    public static void snowFlake(float x, float y, float r, float s) {
        for(int i = 0; i < 3; ++i) {
            Lines.lineAngleCenter(x, y, r + (float)(60 * i), s);
        }
    ...
    public static void spark(float x, float y, float w, float h, float r) {
        for(int i = 0; i < 4; ++i) {
            Drawf.tri(x, y, w, h, r + (float)(90 * i));
        }
    ...
    public static void drawHeat(TextureRegion reg, float x, float y, float rot, float temp) {
        if (temp > 273.15F) {
            ...
            if (a > 1.0F) {
                Color fCol = Pal.turretHeat.cpy().add(0.0F, 0.0F, 0.01F * a);
                fCol.mul(a);
                Draw.color(fCol, a);
            } else {
    ...
    public static void drawSlideRect(TextureRegion region, float x, float y, float w, float h, float tw, float th, float rot, int step, float offset) {
        if (region != null) {
            ...
            TextureRegion var10000 = nRegion;
            var10000.u += Mathf.map(offset % 1.0F, 0.0F, 1.0F, 0.0F, texW * (float)step / tw);
    ...
    public static void drawRotRect(TextureRegion region, float x, float y, float w, float h, float th, float rot, float ang1, float ang2) {
        if (region != null && Core.settings.getBool("effects")) {
            float amod1 = Mathf.mod(ang1, 360.0F);
            float amod2 = Mathf.mod(ang2, 360.0F);
            if (!(amod1 >= 180.0F) || !(amod2 >= 180.0F)) {
                nRegion.set(region);
                float uy1 = nRegion.v;
                float uy2 = nRegion.v2;
                float uCenter = (uy1 + uy2) / 2.0F;
                float uSize = (uy2 - uy1) * h / th * 0.5F;
                uy1 = uCenter - uSize;
                uy2 = uCenter + uSize;
                nRegion.v = uy1;
                nRegion.v2 = uy2;
                float s1 = -Mathf.cos(ang1 * ((float)Math.PI / 180F));
                float s2 = -Mathf.cos(ang2 * ((float)Math.PI / 180F));
                if (amod1 > 180.0F) {
                    nRegion.v2 = Mathf.map(0.0F, amod1 - 360.0F, amod2, uy2, uy1);
                    s1 = -1.0F;
                } else if (amod2 > 180.0F) {
                    nRegion.v = Mathf.map(180.0F, amod1, amod2, uy2, uy1);
                    s2 = 1.0F;
                }

                s1 = Mathf.map(s1, -1.0F, 1.0F, y - h / 2.0F, y + h / 2.0F);
                s2 = Mathf.map(s2, -1.0F, 1.0F, y - h / 2.0F, y + h / 2.0F);
        ...
        if (vertices.length >= 6) {
            for(int i = 2; i < vertices.length; i += 6) {
                ...
                if (!isTriangle) {
                    ...
                    Fill.tri(vertices[0], vertices[1], vertices[i], vertices[i + 1], vertices[i + 2], vertices[i + 3]);
                }
            }
    ...
    public interface Skewer {
        Vec2 get(float var1, float var2, float var3, float var4, float var5, float var6, float var7, float var8, float var9, Vec2 var10);
    }
}
